...
const jornada = [
  {
    image: "/capa_cards/jornada/ilusao.webp",
  ...
  {
    image: "/capa_cards/jornada/desenvolvimento.webp",
  ...
  {
    image: "/capa_cards/jornada/fundacao.webp",
    imageAlign: "right" as const,
    act: "Momento III",
    title: "A Resolução",
  ...
  {
    image: "/capa_cards/jornada/horizonte.webp",
    imageAlign: "left" as const,
    act: "Momento IV",
    title: "O Horizonte",
...
export function WhatIs() {
  return (
    <section id="jornada" className="relative py-24 px-4 overflow-hidden bg-offwhite text-ink">
      <div className="relative max-w-5xl mx-auto">
        ...
        >
          <p className="font-ui text-accent-ink text-xs font-semibold uppercase tracking-[0.2em] mb-4">
            Jornada da Imersão
          </p>
          <h2 className="font-heading text-3xl md:text-5xl font-bold text-ink mb-4 leading-tight">
            Uma tarde que muda sua visão
          </h2>
          <p className="text-current/70 max-w-xl mx-auto text-base md:text-lg">
          ...
          {jornada.map((item, i) => {
            ...
            return (
              <motion.div
                ...
                className="relative overflow-hidden rounded-2xl border border-current/50 hover:border-accent/25 transition-colors duration-300 min-h-[200px] md:min-h-[220px] text-offwhite"
              >
                ...
                <Image
                  ...
                  style={{ objectPosition: isRight ? "right center" : "left center" }}
                />
                ...
                <div
                  className={`absolute inset-0 ${
                    isRight
                      ? "bg-gradient-to-r from-bg-dark from-[58%] md:from-[38%] via-bg-dark/80 md:via-bg-dark/70 via-[82%] md:via-[62%] to-transparent"
                      : "bg-gradient-to-l from-bg-dark from-[58%] md:from-[38%] via-bg-dark/80 md:via-bg-dark/70 via-[82%] md:via-[62%] to-transparent"
                  }`}
                />
                ...
                >
                  <div className={`w-full md:w-[48%] ${isRight ? "" : "text-right"}`}>
                    <div className={`flex items-center gap-2 mb-1.5 ${isRight ? "" : "justify-end"}`}>
                      {item.act && (
                        <p className="font-ui text-accent text-[10px] font-bold uppercase tracking-[0.15em]">
                          {item.act}
                        </p>
                      )}
                    ...
                    <h3 className="font-heading text-lg md:text-xl font-bold text-offwhite mb-0.5 leading-tight">
                      {item.title}
                    </h3>
                    <p className="text-current/40 text-[10px] font-semibold uppercase tracking-wider mb-3">
                      {item.subtitle}
                    </p>
                    ...
                    >
                      {item.items.map((bullet, j) => (
                        <li
                          key={j}
                          className="flex items-start gap-1.5 text-current/65 text-xs leading-snug text-left"
                        >
